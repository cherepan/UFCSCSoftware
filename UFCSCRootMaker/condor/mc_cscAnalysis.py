...
#            print('check indices consistency  ', recoMuIndex,RecoMuonIndexOfThisSegment)
    ...
    def SegmentFinderWithinAConeToMuon(self, tree, good_chambers , simsegment, genMuLV):  # NSegments in the viciinity of Gen Muon
        AllSegments = self.allSegments_InChamber(tree, good_chambers)
        MatchedSegments=[]
...
#            print("in the Cone ??? ")
            ...
            self.sorted_hists1D['MuSegmentdR'].Fill( dR )
            if( dR < 0.2 ): # 0.1
                MatchedSegments.append(segment_index)
        return MatchedSegments


    def RecHitMatcher(self, tree, recosegments, simsegment):
        
        segment = recosegments[0]
        segment_rechits =  self.allRechits_of_segment( tree, segment )
    ...
    def SegmentMatcher(self, tree, good_chambers , simsegment, genMuLV):
...
#        print('--------------------------------- How many segments  ', len(AllSegments))
        for segment_index in AllSegments:
            ...
            segment_rechits =  self.allRechits_of_segment( tree, segment_index )
...
#                                        self.eff_denum_hists1D['SegmentEfficiency_MuonPhi_den'].Fill(GMuLV.Phi())
                                        ...
                                        self.eff_denum_hists1D['MuonSegmentEfficiency_MuonPt_den'].Fill(GMuLV.Pt())
                                        self.eff_denum_hists1D['MuonSegmentEfficiency_MuonEta_den'].Fill(math.fabs(GMuLV.Eta()))
...
#                                        print('--------------- ')
                                        SegmentMatching = self.SegmentMatcher(tree, good_chambers, SimSegment, GMuLV)                                        
                                        SegmentMatched =  SegmentMatching[0]
                                        SegmentMatchedIndex =  SegmentMatching[1]

                                        NSegmentsMatchedToGenMuon = self.SegmentFinderWithinAConeToMuon(tree, good_chambers, SimSegment, GMuLV)
                                        self.sorted_hists1D['NSegmentsMatchedToGenMuon'].Fill(len(NSegmentsMatchedToGenMuon))
#                                        print('matched ? ', SegmentMatched)
                                        #  Find the pull of rec and sim hits 

                                        if(len(allSegmentsInChamber) == 1): 
                                           allrecs=  self.allRechits_of_segment(tree, allSegmentsInChamber[0])
#                                           SegmentDir = TVector3(1,1,1)
#                                           SegmentDir.SetTheta(tree.cscSegments_localTheta[allSegmentsInChamber[0]])
#                                           SegmentDir.SetPhi(tree.cscSegments_localPhi[allSegmentsInChamber[0]])
#                                           SegmentDir.SetMag(1)
...
#                                           deltaT = SimSegmentDir.Theta() - SegmentDir.Theta()
#                                           deltaP = SimSegmentDir.Phi() - SegmentDir.Phi()
#                                           self.sorted_hists1D['MuSegmentdR'].Fill( math.sqrt( math.pow(deltaT,2)  + math.pow(deltaP,2)  ))
#                                           self.sorted_hists1D['MuSegmentdTheta'].Fill(deltaT )
#                                           self.sorted_hists1D['MuSegmentdPhi'].Fill( deltaP )

#                                           print(' dR  ', SimSegmentDir.DeltaR(SegmentDir), '  phi ', SimSegmentDir.Phi() - SegmentDir.Phi() )

#tree.cscSegments_localTheta[segment_index],  ' simHits_theta ', tree.simHits_theta[simsegment[



                                           for rh in allrecs:
                                               rechit_layer   =   tree.recHits2D_ID_layer[rh]
                                               for simhit in SimSegment:
                                                   simhit_layer = tree.simHits_ID_layer[simhit]
                                                   if simhit_layer != rechit_layer: continue
#                                                   print(" pull X", (tree.simHits_localX[simhit] - tree.recHits2D_localX[rh])/ sqrt(tree.recHits2D_localXXerr[rh]) )
#                                                   print(" pull Y", (tree.simHits_localY[simhit] - tree.recHits2D_localY[rh])/ sqrt(tree.recHits2D_localYYerr[rh]) )
                                                   self.sorted_hists1D['SimRecHitsdX'].Fill(tree.simHits_localX[simhit] - tree.recHits2D_localX[rh])
                                                   self.sorted_hists1D['SimRecHitsdY'].Fill(tree.simHits_localY[simhit] - tree.recHits2D_localY[rh])
                                                   self.sorted_hists1D['SimRecHits_PullX'].Fill((tree.simHits_localX[simhit] - tree.recHits2D_localX[rh])/ sqrt(tree.recHits2D_localXXerr[rh]) )
                                                   self.sorted_hists1D['SimRecHits_PullY'].Fill((tree.simHits_localY[simhit] - tree.recHits2D_localY[rh])/ sqrt(tree.recHits2D_localYYerr[rh])  )



                                        if( len(NSegmentsMatchedToGenMuon) !=0 ):
                                            self.sorted_hists1D['MuonSegmentEfficiency_MuonPt'].Fill(GMuLV.Pt())
                                            self.sorted_hists1D['MuonSegmentEfficiency_MuonEta'].Fill(math.fabs(GMuLV.Eta()))
                                            NRecs = self.allRechits_of_segment(tree, NSegmentsMatchedToGenMuon[0])
                                            self.sorted_hists1D['NRH'].Fill( len (NRecs) )
                                            mathcedHits =  self.RecHitMatcher(tree, NSegmentsMatchedToGenMuon, SimSegment)
#                                            print("all NRH ", len(NRecs), "  matched  ", len(mathcedHits) )
                                            self.sorted_hists1D['NRH_matched'].Fill( len (mathcedHits) )
                                        ...
                                        if( SegmentMatched ):
                                            ...
                                            self.sorted_hists1D['DeltaThetaRecoSimHits'].Fill(tree.cscSegments_localTheta[SegmentMatchedIndex] - tree.simHits_theta[SimSegment[0]])
                                            self.sorted_hists1D['DeltaPhiRecoSimHits'].Fill(tree.cscSegments_localPhi[SegmentMatchedIndex]     - tree.simHits_phi[SimSegment[0]])
...
#                                print '  EC  ', ec,'  st:  ', st ,' rg:  ', rg, ' ch:  ', ch,'  layer:  ',lr, '   nRecHits:  ',self.nRecHitsPerLayer[ec][st][rg][ch][lr] 
    ...
    def defineHistos(self):
        ...
        self.sorted_hists1D['NRH']                         = ROOT.TH1F("NRH", ";N RecHits in Segment ",10,-0.5,9.5)
        self.sorted_hists1D['NRH_matched']                         = ROOT.TH1F("NRH_matched", ";N RecHits in Segment matched to Sim ",10,-0.5,9.5)
...
#        self.sorted_hists1D['SimRecHitsDifference']  = ROOT.TH1F("SimRecHitsDifference","; NSimHits - NRecHits", 14,-5.5, 8.5) 
        self.sorted_hists1D['NSegmentsSelectedChamber'] = ROOT.TH1F("NSegmentsSelectedChamber","; N Segments in selected chamber (with at least 3 #mu simhits)", 10, -0.5,9.5)
        ...
        self.sorted_hists1D['DeltaThetaRecoSimHits']  = ROOT.TH1F("DeltaThetaRecoSimHits","; #Delta#theta (rechit-simhit), rad", 60,-0.5, 0.5)
        self.sorted_hists1D['DeltaPhiRecoSimHits']    = ROOT.TH1F("DeltaPhiRecoSimHits","; #Delta#phi (rechit-simhit), rad", 60,-0.1, 0.1)
...
#        self.sorted_efficiency['SegmentEfficiency_MuonPt']     = ROOT.TEfficiency("SegmentEfficiency_MuonPt","; pT (gen #mu), GeV ",30,25,70)
        ...
        self.eff_denum_hists1D['MuonSegmentEfficiency_MuonPt_den'] = ROOT.TH1F("MuonSegmentEfficiency_MuonPt_den","; pT (gen #mu) ",30,25,70)
        self.sorted_hists1D['MuonSegmentEfficiency_MuonPt']        = ROOT.TH1F("MuonSegmentEfficiency_MuonPt", "; pT (gen #mu), GeV ",30,25,70)
        self.sorted_efficiency['MuonSegmentEfficiency_MuonPt']     = ROOT.TEfficiency("MuonSegmentEfficiency_MuonPt","; pT (gen #mu), GeV ",30,25,70)

        self.eff_denum_hists1D['MuonSegmentEfficiency_MuonEta_den'] = ROOT.TH1F("MuonSegmentEfficiency_MuonEta_den","; |#eta| (gen #mu)",30,1.0,2.5)
        self.sorted_hists1D['MuonSegmentEfficiency_MuonEta']        = ROOT.TH1F("MuonSegmentEfficiency_MuonEta", "; |#eta| (gen #mu) ",30,1.0,2.5)
        self.sorted_efficiency['MuonSegmentEfficiency_MuonEta']        = ROOT.TEfficiency("MuonSegmentEfficiency_MuonEta",";  |#eta| (gen #mu) ",30,1.0,2.5)
        ...
        self.sorted_hists1D['SegmentMatchingPullX'] = ROOT.TH1F('SegmentMatchingPullX',"; Sim-Rec hits  X (mc - rec)/#sigma", 50,-10, 10)
        self.sorted_hists1D['SegmentMatchingPullY'] = ROOT.TH1F('SegmentMatchingPullY',"; Sim-Rec hits  Y (mc - rec)/#sigma", 50,-10, 10)

        self.sorted_hists1D['MuSegmentdR'] = ROOT.TH1F('MuSegmentdR',"; dR (RecSegment - Gen #mu)  ", 50, 0, 0.6)
        self.sorted_hists1D['MuSegmentdPhi'] = ROOT.TH1F('MuSegmentdPhi',"; d#phi (RecSegment - Gen #mu)  ", 50, -0.3, 0.3)
        self.sorted_hists1D['MuSegmentdTheta'] = ROOT.TH1F('MuSegmentdTheta',"; d#theta (RecSegment - Gen #mu)  ", 50, -0.6, 0.6)


        self.sorted_hists1D['NSegmentsMatchedToGenMuon'] = ROOT.TH1F('NSegmentsMatchedToGenMuon',"; N segments in the #mu cone  ", 4, -0.5,3.5)
...
#        self.eff_denum_hists1D['SegmentEfficiency_MuonPt_ME11_den'] = ROOT.TH1F("SegmentEfficiency_MuonPt_ME11_den","; #phi (gen #mu) ",30,25,70)
#        self.sorted_hists1D['SegmentEfficiency_MuonPt_ME11']           = ROOT.TH1F("SegmentEfficiency_MuonPt_ME11", "; pT (gen #mu), GeV ",30,25,70)
#        self.sorted_efficiency['SegmentEfficiency_MuonPt_ME11']        = ROOT.TEfficiency("SegmentEfficiency_MuonPt_ME11","; pT (gen #mu), GeV ",30,25,70)
...
#        self.sorted_efficiency['SegmentEfficiency_MuonEta_ME11']        = ROOT.TEfficiency("SegmentEfficiency_MuonEta_ME11","; |#eta| (gen #mu) ",30,1.0,2.5)
        ...
        # pulls
        self.sorted_hists1D['SimRecHits_PullX'] = ROOT.TH1F('SimRecHits_PullX',"; pull X (mc - rec)/#sigma ", 50,-5, 5)
        self.sorted_hists1D['SimRecHits_PullY'] = ROOT.TH1F('SimRecHits_PullY',"; pull Y (mc - rec)/#sigma)", 50,-5, 5)

        self.sorted_hists1D['SimRecHitsdX'] = ROOT.TH1F('SimRecHitsdX',"; Segment #Delta X (mc - rec)", 50,-0.5, 0.5)
        self.sorted_hists1D['SimRecHitsdY'] = ROOT.TH1F('SimRecHitsdY',"; Segment #Delta Y (mc - rec)", 50,-2, 2)
...
#        self.sorted_hists1D['SegmentEfficiency_MuonPt'].Divide(self.eff_denum_hists1D['SegmentEfficiency_MuonPt_den'])
        ...
        self.sorted_efficiency['MuonSegmentEfficiency_MuonPt'] = ROOT.TEfficiency(MuonSegmentEfficiency_MuonPt,MuonSegmentEfficiency_MuonPt_den)
        self.sorted_hists1D['MuonSegmentEfficiency_MuonPt'].Sumw2()
        self.sorted_hists1D['MuonSegmentEfficiency_MuonPt'].Divide(self.eff_denum_hists1D['MuonSegmentEfficiency_MuonPt_den'])

        self.sorted_efficiency['MuonSegmentEfficiency_MuonEta'] = ROOT.TEfficiency(MuonSegmentEfficiency_MuonEta,MuonSegmentEfficiency_MuonEta_den)
        self.sorted_hists1D['MuonSegmentEfficiency_MuonEta'].Sumw2()
        self.sorted_hists1D['MuonSegmentEfficiency_MuonEta'].Divide(self.eff_denum_hists1D['MuonSegmentEfficiency_MuonEta_den'])
